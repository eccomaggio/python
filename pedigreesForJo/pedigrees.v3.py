...
#container {{
    ...
    border-left: var(--grid-border-width) solid black;
    border-bottom: var(--grid-border-width) solid black;
}}

#container > div {{
    display: flex;
    align-items: stretch;
    border-right: var(--grid-border-width) solid black;
}}

.sire {{
    border-top: var(--grid-border-width) solid black;
}}

.dam {{
    border-top: var(--grid-border-width) dashed black;
    }}
"""
...
def build_gen1(cat_id, pedigree,i):
    return build_generic(cat_id, pedigree)


def build_gen2(cat_id, pedigree, i):
    return build_generic(cat_id, pedigree)


def build_gen3(cat_id, pedigree, i):
    # i = get_display_info(cat_id, pedigree)
    return f"""
    <p class="nameline{i["awards_class"]}">{i["awards"]}
        {i['name']}
...
def build_gen4(cat_id, pedigree, i):
    # i = get_display_info(cat_id, pedigree)
    return f"""
    <p class="nameline{i["awards_class"]}">{i["awards"]}
        {i['name']}
...
def build_gen5(cat_id, pedigree, i):
    # i = get_display_info(cat_id, pedigree)
    return f"""
...
#     return tmp
def build_html_cat(g_id, cat_id, pedigree):
    info = get_display_info(cat_id, pedigree)
    tmp = f"""
<div class="cat {info['sex'].lower()}">
    {select_html_template(g_id, cat_id, pedigree, info)}
</div>
    """
    return tmp
